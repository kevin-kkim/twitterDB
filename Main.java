...
public class Main {
    ...
    public static void main(String args[]) throws InterruptedException, UnknownHostException, TwitterException {
        ...
        UserStreamListener listener = new UserStreamListener() {
            ...
            public void onStatus(Status _status) {
                ...
                document_a.append("tweet_user", _status.getUser().getScreenName());
                document_a.append("text", _status.getText());
                collection_a.insertOne(document_a);
                System.out.println(document_a);

                // put tweets into the Map
                puttweets(_status.getText());
            ...
            public void onFollow(User source, User followedUser) {
                System.out.println("onFollow source:@" + source.getScreenName() + " target:@"
    ...
    private static void connectmongoDB_a(MongoClient _mongoClient, MongoDatabase _database, String _dbname,
            String _colname) {
        // Connect to MongoDB
        ...
        collection_a = _database.getCollection(_colname);
    }

    @SuppressWarnings("resource")
    private static void connectmongoDB_b(MongoClient _mongoClient, MongoDatabase _database, String _dbname,
            String _colname) {
        // Connect to MongoDB
        _mongoClient = new MongoClient();
        // Accessing Database
        _database = _mongoClient.getDatabase(_dbname);
        // Retrieving a collection
        collection_b = _database.getCollection(_colname);
    }

    private static void puttweets(String _tweet) {

        String[] words = _tweet.split(" ");
        for (int i = 0; i < words.length; i++) {
            tweets.put(words[i], _tweet);
        }
    }
    ...
    private static void createDocument(String _key, LinkedList<String> _value) {
        // Create new document instance
        Document document = new Document();
        // append the data into the document
        document.append("Key", _key);
        document.append("Value", _value);
